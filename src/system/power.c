 ...
 * details before the system reboots. Error details are saved to retained RAM
 * for display by the `lastreset` shell command after reboot.
 *
 * Placed in power.c (not a separate file) to avoid LTO multiple-definition
 * conflicts in sysbuild CI configurations.
 */
